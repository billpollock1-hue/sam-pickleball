...
the manual 4 AM MST / 6 AM CDT routine:
  ...
  4. Check In All.
  5. Audit DEN's own Step/% seeding against the same logic den_assignments.py
     already trusts (assign_courts), and correct any Ladder Step box where
     DEN disagrees -- including "Den New Player Tryout" mis-seeds.
  6. Seed Players.
  7. Start Event.
...
Reuses helpers from den_assignments.py rather than re-deriving them:
  ...
  - assign_courts   (ground truth for correct Step seeding, incl. the
                      Den New Player Tryout force-to-bottom rule)
...
def cross_check_and_correct_seeding(page, computed_assignments):
    """
    computed_assignments: the DataFrame returned by assign_courts() --
    already-trusted logic that includes the Den New Player Tryout
    force-to-bottom rule. This function audits every row DEN seeded and
    overwrites only the ones that disagree.
...
def main():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            print("Opening ratings page for the seeding audit...")
            page.goto(RATINGS_URL, wait_until="domcontentloaded")
            page.wait_for_timeout(1500)
            ratings_text = save_page_text(page, "ratings_page_create.txt")

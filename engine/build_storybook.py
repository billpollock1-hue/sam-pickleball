...
<p><b>S1 Spread</b>, <b>S2 Spread</b>, and <b>Combined Spread</b> are the average within-court rating gaps for Shootout 1, Shootout 2, and the two combined — lower means more evenly matched courts. <b>vs Den</b> converts the combined figure into a single improvement percentage versus today’s system.</p>
...
<p>We also tested a fast dial, which pushes the same trade further. It buys a little more movement — closer to today’s level, though still well below it — but the additional loss in court balance outweighs what that extra movement is worth, so balanced dial, not fast dial, is the recommendation.</p>
...
<p>The two phases differ in more than scope, though. Phase 1 can be run overnight and finish before anyone checks in — the seeding is already loaded and waiting when the first player arrives. Phase 2 can’t work that way: it has to fire live, on the spot, in the gap between Shootout 1’s last game and Shootout 2’s first. That makes its remaining hurdle a question of who’s there to trigger it, not whether the code works.</p>
</div>
